...
低秩扰动，对 10 类 MNIST 训练集的拟合精度上限本就有限）。

**⑤ 框架级改动探索**（授权放宽框架后，`exp_fullrank_es.py` / `exp_layered_full.py`）：

| 改动 | best_train | val_acc |
|---|---|---|
| 全满秩 ES（fc1/fc2/fc3 满秩，batch=512）| 0.5918 | 0.6309 |
| fc3 满秩 + fc1/fc2 LoRA rank72（batch=6000）| 0.8198 | **0.8691** |
| 全 LoRA rank72（对照，最高 best_train）| **0.8335** | 0.8486 |

→ **框架级改动同样无法突破 best_train≈0.83**：
1. **全满秩 ES 崩溃（0.59）**：满秩扰动把 fc1 权重维度抬到 ~150k，纯 ES 在如此高维
   上搜索**样本效率极低**，收敛远慢于低维 LoRA。
2. **fc3 满秩（分层自由度）**：把输出层（仅 1280 参数）改为满秩后，**val 升到 0.8691
   （本项目新高）**，但 **best_train 反而降到 0.8198**——满秩输出层学出了更平滑的
   决策边界（泛化↑），却不"死记"训练样本（训练集峰值拟合↓）。

**最终结论（重要）**：`best_train`（训练集拟合峰值）与 `val_acc`（泛化）在本框架下
存在**内在权衡**。经过 **参数调优（6 方向）+ 框架改动（满秩/分层自由度）** 的完整探索，
`best_train` 始终封顶 ~0.833，**无法达到 0.9+**。根因是**纯演化策略（ES）无梯度、
仅靠随机扰动 + 适应度加权来更新权重**，其对训练集的精确拟合能力（记住 90% 样本）存在
本质上限；即便解除 LoRA 低秩限制，又会撞上"高维 ES 搜索效率崩溃"的另一堵墙。
**要突破 0.9 必须引入梯度信息（如 SNN 代理梯度/反向传播监督）或改用基于梯度的训练**，
这已超出"演化策略"框架本身。

**⑥ STDP（脉冲时序依赖可塑性）探索**（`exp_stdp.py`，自包含实现，fc1/fc2 用 STDP、
fc3 用 softmax 回归读出）：

| 变体 | best_train | val_acc |
|---|---|---|
| Poisson 输入 + 无监督 STDP | 0.1335 | 0.0977 |
| Poisson 输入 + 监督 STDP | 0.1125 | 0.0977 |
| 确定性 rate 输入 + 监督 STDP（[128,128]）| 0.4098 | 0.3457 |
| 确定性 rate 输入 + 监督 STDP（[256,256], 10000ep）| 0.4233 | 0.2578 |

→ **关键发现：输入编码是 STDP 有效性的决定性因素**。
1. **Poisson 随机尖峰输入下 STDP 完全失效（~0.11，随机水平）**：Poisson 编码的尖峰
   逐时间步独立随机、**无时间结构**，STDP 学到的 pre/post 时序关系是噪声而非判别特征。
2. **改用确定性 rate 输入后 STDP 开始学习（best_train 0.11→0.41，3.7 倍）**：每个时间步
   输入相同像素值（持续电流），pre/post 尖峰时序关系稳定，STDP 才能学到有意义的特征。
3. **但即便 rate 输入 + 更大网络（[256,256]），best_train 也仅 0.42、平台不前**：
   简单 STDP + softmax 读出的方案在 2 层网络下**封顶 ~0.42**，远低于 ES 的 0.833，且 val
   大幅波动（0.09~0.37）不稳定。经典 STDP MNIST（如 Diehl & Cook）需**大兴奋层
   （~6400 神经元）+ 侧抑制（winner-take-all）+ 阈值自适应**等复杂机制才能到 ~0.95。
   要把 STDP 做到 0.9 需大型网络重构，非简单叠加所能达成。

**⑦ ES + STDP 混合探索**（`exp_es_stdp_hybrid.py`，低秩 ES 全局梯度 + STDP 满秩局部梯度叠加）：

| 方案 | best_train | val_acc |
|---|---|---|
| ES(LoRA) + STDP 梯度直接叠加 | 0.1225 | 0.0977 |

→ **关键发现：ES 与 STDP 不能简单梯度叠加（混合后崩溃到随机水平 0.12）**。
   根因是两者**优化目标不同**：ES 的 LoRA 梯度由 `loglik fitness`（分类正确性）驱动，
   STDP 的满秩梯度由 **pre/post 尖峰时序的 Hebbian 因果性**驱动。两者方向不兼容，
   直接相加把权重拉向矛盾方向，最终崩溃（比纯 ES 0.833 和纯 STDP 0.42 都差）。
   真正有效的结合需要让 STDP 严格服务 ES 的分类目标（而非独立 Hebbian），或采用
   "ES 预训练 → STDP 精调"的分阶段方案，而非同一步梯度叠加。

> 说明：以上探索均保留在脚本与 CSV（`exp_besttrain_sweep.py` / `exp_besttrain_long.py` /
> `exp_besttrain_highlr.py` / `exp_besttrain_lrsweep.py` / `exp_fullrank_es.py` /
> `exp_layered_full.py` / `exp_stdp.py` / `exp_es_stdp_hybrid.py` 及对应 `results_*.csv`），
> 可用作后续框架改进的基线参照。
