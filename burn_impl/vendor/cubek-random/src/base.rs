...
pub(crate) fn random<F: RandomFamily, R: Runtime>(
    ...
    // 本项目 patch：向量化写（每线程一次写 Vector<E,4>，16 字节对齐）。
    // RNG 状态序列与 N=1 完全一致（inner_loop 逐元素推进状态），数值逐位不变，
    // 只是写指令数降为 1/4，实测 f32 随机吞吐 ~260GB/s -> ~700GB/s。
    // 要求输出尺寸能被 4 整除（所有调用方的形状均满足；不满足时退回 1）。
    let output_vector_size = if output.size() % 4 == 0 { 4 } else { 1 };
