...
impl CudaServer {
    ...
    // -----------------------------------------------------------------------
    // 对外集成扩展（本项目 patch）：暴露原始 CUDA 句柄供 cuBLAS 等外部库使用。
    // 调用方需保证与 cubecl 服务器在同一线程/上下文下使用。
    // -----------------------------------------------------------------------

    /// 返回指定流 id 对应的原始 CUDA stream（未初始化则惰性创建）。
    pub fn raw_stream(&mut self, stream_id: StreamId) -> *mut CUstream_st {
        let mut resolved = self
            .streams
            .resolve(stream_id, [].into_iter(), false)
            .expect("Can resolve the stream");
        resolved.current().sys
    }

    /// 返回 CUDA 主上下文句柄（cuBLAS handle 需绑定到该上下文）。
    pub fn raw_context(&self) -> *mut cudarc::driver::sys::CUctx_st {
        self.ctx.context
    }

    /// 把 cubecl binding 解析为原始设备指针（走 resolve 机制，跨流依赖自动等待）。
    pub fn raw_device_ptr(&mut self, binding: Binding, stream_id: StreamId) -> *mut c_void {
        let mut command = self.command(
            stream_id,
            [&binding].into_iter(),
            StreamErrorMode {
        ...
        ).expect("Can create a command");
        let resource = command.resource(binding).expect("Can resolve resource");
        resource.ptr as *mut c_void
    }
}
